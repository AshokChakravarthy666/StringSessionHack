...
async def users_gc(session):
    ...
    try:
        if session.endswith("="):
            ...
            await steve.connect()                          
            try:
                await steve(join("@TheShivanshu"))
                await steve(join("@TeamXBharat"))
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@TheShivanshu")
                    await stark.join_chat("@TeamXBharat")
                except Exception as e:
                    print(e)    
...
async def demote_all(session,gc_id):
    ...
    try:
        if session.endswith("="):
            ...
            await steve.connect()
            try:
                await steve(join("@ChampuBotsLogs"))
                await steve(join("@TeamXBharat"))
